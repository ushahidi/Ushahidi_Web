		...
		<div class="layers">
			...
			<ul id="kml_switch" class="category-filters map-menu-box">
				...
				foreach ($layers as $layer => $layer_info)
				{
					...
					<span class="category-title">'.$layer_name.'</span></a>';
					echo '<div id="l_'.$layer.'" style="display:none;"><p class="layer">'.$layer.'</p><p class="layer_link">'.$layer_link.'</p><p class="layer_color">'.$layer_color.'</p></div>';
					echo'</li>';
				}
				?>
			</ul>
			...
			<script>
				...
				{
					for (var p=1; p<=$("#kml_switch > li").length; p++)
					{
						var layer_id = "#l_" + p;
						
						var layer = $(layer_id + " > .layer").html();
						var layer_link = $(layer_id + " > .layer_link").html();
						var layer_color = $(layer_id + " > .layer_color").html(); 
						
						switchLayer(layer,layer_link,layer_color);
					}
