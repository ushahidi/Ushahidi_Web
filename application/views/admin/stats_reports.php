...
<div class="bg">
	...
	<div>
		...
		<div style="border:solid 2px #444444;margin-left:25px;width:500px;">
			<table>
				<tr><th>Date</th>
				<?php
				$date_range = array();
				$keys = array();
				foreach($raw_approved_verified_data as $name => $data) {
					echo '<th>'.$name.'</th>';
					$keys[] = $name;
				...
				foreach($date_range as $timestamp) {
					...
					foreach($keys as $name) {
						echo '<td>'.$raw_approved_verified_data[$name][$timestamp].'</td>';
					}
					echo '</tr>';
				}
				?>
		...
		<div style="border:solid 2px #444444;margin-left:25px;width:600px">
			<table>
				<tr><th>Date</th>
				<?php
				$date_range = array();
				$keys = array();
				foreach($raw_category_data as $name => $data) {
					echo '<th>'.$name.'</th>';
					$keys[] = $name;
					if(count($date_range) == 0) {
						foreach($data as $timestamp => $arr) $date_range[] = $timestamp;
				...
				foreach($date_range as $timestamp) {
					echo '<tr><td>'.date('M jS, Y',$timestamp).'</td>';
					foreach($keys as $name) {
						echo '<td>'.$raw_category_data[$name][$timestamp].'</td>';
					}
					echo '</tr>';
				}
				?>
