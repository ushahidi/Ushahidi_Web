...
class Json_Controller extends Template_Controller
{
	...
	{		
		...
		if ( is_numeric($category_id) && $category_id != 0 )
		{
			...
			foreach (ORM::factory('incident')->join('incident_category', 'incident.id', 'incident_category.incident_id','INNER')->select('incident.*')->where('incident.incident_active = 1 AND incident_category.category_id = ' . $category_id)->orderby('incident.incident_dateadd', 'desc')->find_all() as $marker)
			{			
				$json_item = "{";
