...
class Alerts_Controller extends Main_Controller {
	...
	*/
	function confirm ()
	{
		...
		{
			$alert_response .= "<div class=\"alert_response\">Your [MOBILE] Alert request has been created and a verification message has been sent to <u><strong>" . $_SESSION['alert_mobile'] . "</strong></u> You will not receive Alerts on this location until you enter the verification code below to confirm your request. " . $this->_form_confirm_mobile() . "</div>";
		}
		
		if (!empty($_SESSION['alert_email']))
		{
			$alert_response .= "<div class=\"alert_response\">Your [EMAIL] Alert request has been created and a verification message has been sent to <u><strong>" . $_SESSION['alert_email'] . "</strong></u> You will not receive Alerts on this location until you click the link in the verification email and confirm your request.</div>";
		}
		
		$alert_response .= "<div class=\"alert_response\"><a href=\"" . url::base() . 'alerts' . "\">Return to the Alerts page to create more alerts</a>.</div>";

		$this->template->content->alert_response = $alert_response;
	}
	...
	*/
	public function _mobile_check(Validation $post)
	{
	    ...
	    if (array_key_exists('alert_mobile', $post->errors()) || array_key_exists('alert_lat', $post->errors()) || array_key_exists('alert_lon', $post->errors()))
	        return;

	    // Now check for similar alert in system
		$mobile_check = ORM::factory('alert')->where('alert_type', '1')->where('alert_recipient', $post->alert_mobile)->where('alert_lat', $post->alert_lat)->where('alert_lon', $post->alert_lon)->find();
		if ( $mobile_check->id )
	    {
	...
	*/
	...
	{
	    // If add->rules validation found any errors, get me out of here!
	    if (array_key_exists('alert_email', $post->errors()) || array_key_exists('alert_lat', $post->errors()) || array_key_exists('alert_lon', $post->errors()))
	        return;

	    // Now check for similar alert in system
		$email_check = ORM::factory('alert')->where('alert_type', '2')->where('alert_recipient', $post->alert_email)->where('alert_lat', $post->alert_lat)->where('alert_lon', $post->alert_lon)->find();
		if ( $email_check->id )
	    {
	        // Add a validation error, this will cause $post->validate() to return FALSE
	...
	/*
	* Generates Mobile Confirmation Form
	*/
	private function _form_confirm_mobile()
	{
