	...
	*/
    ...
    function geoGetCoords($address,$depth=0) {
        switch($this->lookup_service) {
            case 'GOOGLE':
                $_url = sprintf('http://%s/maps/api/geocode/json?sensor=%s&address=%s,%s&region=lr',$this->lookup_server['GOOGLE'], $this->mobile==true?"true":"false", rawurlencode($address),"%20liberia");
                $_result = false;
